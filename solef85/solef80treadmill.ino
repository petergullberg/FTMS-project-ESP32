...
void waitForAPacket() {
  ...
  uint16_t len;
  // probabbly wrong
    if ( wait_response(cmd_buf, &len, 1000) ) {
      Serial.printf("Success\n");
      return;
    };
}
void writeCharacteristic(uint8_t *data, uint8_t data_len, const char *info, bool disable_log,
                                           bool wait_for_response) 
{
  disable_log=0;
  if ( !disable_log) 
    Serial.printf("TX: %s\n", info);
  send_command(data,data_len);
  if (wait_for_response ) {
  if ( !disable_log) 
    Serial.printf("RX: %s\n", info);
    uint8_t cmd_buf[129];
    uint16_t cmd_buf_len;
    if ( wait_response(cmd_buf, &cmd_buf_len, 4000) ) {
      if ( !disable_log)  {
        analyze_print_resp(cmd_buf, cmd_buf_len);

        Serial.printf("Success\n");
      }

    } else {
        Serial.printf("could not receive response\n");
    }
  }
  //delay(500);
}
                                           ...
                                           bool wait_for_response) 
{
  ...
  len = strlen(cmd_str);
  hexStr2bytes(cmd_str, cmd_buf, len); 
  writeCharacteristic(cmd_buf, len/2, info, disable_log, wait_for_response);
}

int initDone;
#define QStringLiteral(x) x
void solef80treadmill_btinit(void) {

    int settings=0;
    bool f65 = 0;
    bool f63 = 0;
    bool tt8 = 0;
    int gattCustomService = 1;
...
void solef80treadmill::writeCharacteristic(uint8_t *data, uint8_t data_len, const char *info, bool disable_log,
                                           bool wait_for_response) {
    QEventLoop loop;
    QTimer timeout;
    QSettings settings;
